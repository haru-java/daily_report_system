/*
Lesson 17Chapter 5.1
フロントコントローラの作成

以下のよう action の値が不正（存在しないクラス名）で、該当するActionクラスがない場合はエラーが発生しますので
catch して　エラー画面を表示するための処理を行います。
http://localhost:8080/daily_report_system/?action=xxxyyyzzz&command=index
 */
package controllers;
